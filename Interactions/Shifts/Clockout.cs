...
{
    public class Clockout : InteractionData
    {
        ...
        public override async Task ExecuteAsync(InteractionContext ctx)
        {
            ...
            var (shift, errormessage) = await Procedures.Clockout(long.Parse(ctx.GuildId), long.Parse(ctx.UserId), thisType);
            ...
            else
            {
                ...
                await ctx.EditMessage(
                    data.generateMessage(ctx.UserId, thisType, true, shift)
                );
            }
        }
    }
